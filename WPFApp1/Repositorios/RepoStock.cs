                ...
                using (SqliteCommand comando = new SqliteCommand(Consulta, _accesoDB.ObtenerConexionDB()))
                {
                    ...
                    using (SqliteDataReader lector = comando.ExecuteReader())
                    {
                        if (lector.Read())
                        {
                            ...
                            int IDXCondicionNombre = lector.GetOrdinal("CondicionNombre");
                            ...
                            registro.CondicionNombre = lector.IsDBNull(IDXCondicionNombre) ? "" : lector.GetString(IDXCondicionNombre);
                    ...
                    using (SqliteDataReader lector = comando.ExecuteReader())
                    {
                        ...
                        while (lector.Read())
                        {
                            ProductoCatalogo registroActual = new ProductoCatalogo
                            {
                                ...
                                CondicionNombre = lector.IsDBNull(IDXCondicionNombre) ? "" : lector.GetString(IDXCondicionNombre),
            ...
            using (SqliteConnection conexion = _accesoDB.ObtenerConexionDB())
            {
                using (SqliteCommand comando = new SqliteCommand())
                {
                    ...
                    foreach (Propiedad_Valor filtro in Busqueda)
                    {
                        ...
                        if(filtro.PropiedadNombre == "Haber")
                        {
                            if(valorParametro == "1")
                            {
                                listaPropiedadesBuscadas.Add($"{nombreIndice} > @{filtro.PropiedadNombre}");
                                comando.Parameters.AddWithValue($"@{filtro.PropiedadNombre}", "0");
                            }
                            else
                            {
                                listaPropiedadesBuscadas.Add($"{nombreIndice} < @{filtro.PropiedadNombre}");
                                comando.Parameters.AddWithValue($"@{filtro.PropiedadNombre}", "1");
                            }
                        }
                        else
                        {
                    ...
                    if(listaPropiedadesBuscadas.Count > 0)
                        Consulta += $" WHERE {string.Join(" AND ", listaPropiedadesBuscadas)};";
